...
async function fetchETF(symbol) {
  try {
    // 1y range gives us: 52W high, 60D spark (needed for Aroon-34), 30D change, volume avg
    ...
    // 60-point spark from valid closes (extended from 30 for Aroon-34 + EMA-21)
    const spark     = valid.slice(-60).map(x => x.c)
    // High/low arrays for Aroon (same length as spark)
    const sparkHigh = valid.slice(-60).map(x => x.h)
    const sparkLow  = valid.slice(-60).map(x => x.l)

    return {
      price: parseFloat(price.toFixed(2)),
      change30d, drawdown, volRatio, spark, sparkHigh, sparkLow,
...
export async function GET() {
  ...
  const data = {}
  for (const [sym, d] of results) { if (d) data[sym] = d }
